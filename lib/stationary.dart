...
class stationaryclass  extends StatelessWidget{
  List <Stat> dataofstationary = [];
  ...
  Future<List<Stat>> getdata13() async {


    final response = await http.get(
        Uri.parse('https://4ba9c079-ff54-425d-8451-3afd3fa3fc53.mock.pstmn.io/stationary'));
    ...
    if (response.statusCode == 200) {
      ...
      for (Map<String, dynamic> index in save) {
        dataofstationary.add(Stat.fromJson(index));
      }

      return dataofstationary; // Return the list of mobile objects
    } else {
      return dataofstationary; // Return an empty list in case of an error
    }
  }
                      ...
                      children: [
                        ...
                        InkWell(onTap: (){
                          Navigator.push(context, MaterialPageRoute(builder: (context){
                            return mobilesection(key:GlobalKey(),name:"${dataofstationary[index].name}", image:dataofstationary[index].image,description:dataofstationary[index].description,price:dataofstationary[index].price, brandname:dataofstationary[index].brandname,setindexvalue:index );
                          }));
...
String statToJson(List<Stat> data) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
